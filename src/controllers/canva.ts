...
export default class CanvaController {
  ...
  initSockets(socket: Socket, userId: number, username: string) {
    ...
    socket.on(
      this.scope + 'new-pixel:' + this.id,
      async (data: any, index: number, position: Coord, color: string) => {
        ...
        if (isValid) {
          ...
          socket
            .to(this.roomId)
            .emit(this.scope + 'new-pixel-from-others', position, color);
    ...
    socket.on(
      this.scope + 'new-pixels:' + this.id,
      async (data: any, pixels: Pixels) => {
        ...
        if (isValid) {
          socket
            .to(this.roomId)
            .emit(this.scope + 'new-pixels-from-others', pixels);
